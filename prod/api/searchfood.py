...
def gen_recommendations(uid, response_data):
    ...
    if profile is not None:
        ...
        weights_dict = {}
        attrs = {}  # attributes for explainability
        for food in relevant_response_fields:
            attrs[food['food']['foodId']] = []
            ...
            if bmi_w == 'underweight':
                if 'ENERC_KCAL' in food['food']['nutrients']:
                    ...
                    if float(food['food']['nutrients']['ENERC_KCAL']) > 300:
                        weights_dict[food_tuple] += 3.0
                        attrs[food['food']['foodId']].append('High calories')
            elif bmi_w == 'normal':
                if 'ENERC_KCAL' in food['food']['nutrients']:
                    # filter by 'medium calorie' foods.
                    if float(food['food']['nutrients']['ENERC_KCAL']) > 200 and float(food['food']['nutrients']['ENERC_KCAL']) < 300:
                        weights_dict[food_tuple] += 1.0
                        attrs[food['food']['foodId']].append('Moderate calories')
            elif bmi_w == 'overweight':
                if 'ENERC_KCAL' in food['food']['nutrients']:
                    # filter by 'lower calorie' foods.
                    if float(food['food']['nutrients']['ENERC_KCAL']) < 200:
                        weights_dict[food_tuple] += 3.0
                        attrs[food['food']['foodId']].append('Low calories')
            ...
            if 'PROCNT' in food['food']['nutrients']:
                ...
                if percentage_protein >= 10.0:
                    ...
                    attrs[food['food']['foodId']].append('High protein')
            
        date = datetime.date.today()
        date_str = date.strftime("%Y-%m-%d")
        for food in relevant_response_fields:
            insert_search_history(uid, food['food']['label'], date_str, weights_dict[(food['food']['foodId'], food['food']['label'])], str(attrs[food['food']['foodId']]))
    else:
